...
import ChartsExample from './examples/ChartsExample.vue';
import EmptyExample from './examples/EmptyExample.vue';
import FormExample from './examples/FormExample.vue';
import NotificationsExample from './examples/NotificationsExample.vue';
import TablesExample from './examples/TablesExample.vue';
import TimelineExample from './examples/TimelineExample.vue';
//import { Sun, Moon } from 'lucide-vue-next';

const tabs = ['Calendar', 'Charts', 'Empty', 'Form', 'Notifications', 'Tables', 'Timeline'];
    ...
    <div :class="['min-h-screen transition-colors duration-300', isDarkMode ? 'bg-gray-900' : 'bg-gray-50']">
            ...
            <div class="max-w-full sm:max-w-7xl mx-auto px-2 sm:px-4 lg:px-8">
                <div class="flex flex-wrap md:flex-nowrap justify-between h-16 items-center gap-2">
                    <div class="flex items-center gap-2">
                        <h1 class="text-2xl font-bold text-primary-600">BsaAddons</h1>
        ...
        <div class="w-full max-w-full sm:max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-8">
            <transition name="page-fade" mode="out-in">
                <div v-if="activeTab === 'Empty'" class="overflow-x-auto">
                    <EmptyExample />
                </div>
                <CalendarExample v-else-if="activeTab === 'Calendar'" />
                <ChartsExample v-else-if="activeTab === 'Charts'" />
                <EmptyExample v-else-if="activeTab === 'Empty'" />
                <FormExample v-else-if="activeTab === 'Form'" />
                <NotificationsExample v-else-if="activeTab === 'Notifications'" />
                <TablesExample v-else-if="activeTab === 'Tables'" />
                <TimelineExample v-else-if="activeTab === 'Timeline'" />
            </transition>
        </div>
